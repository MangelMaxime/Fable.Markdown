...
Target.create "Build" (fun _ ->
    let proc = DotNet.exec id "fable" "webpack --port free -- -p"
    if proc.ExitCode <> 0 then
        failwithf "Dotnet existed with code: %i\n" proc.ExitCode
)

Target.create "Watch" (fun _ ->
    let proc = DotNet.exec id "fable" "webpack-dev-server --port free"
    if proc.ExitCode <> 0 then
        failwithf "Dotnet existed with code: %i\n" proc.ExitCode
)
...
Target.create "RunLiveTests" (fun _ ->
    let proc = DotNet.exec id "fable" "yarn-run webpack-dev-server --port free -- --config tests/webpack.config.js"
...
Target.create "BuildTests" (fun _ ->
    let proc = DotNet.exec id "fable" "yarn-run webpack --port free -- -p --config tests/webpack.config.js"
