...
module ParserLib
    ...
    let satisfy predicate label =
        let innerFn input =
            ...
            match charOpt with
            | None ->
                printfn "satisfy no mlore input"
                let err = "No more input"
                let pos = parserPositionFromInputState input
                Failure (label,err,pos)
            | Some first ->
                printfn "satisfy keep"
                if predicate first then
                    printfn "satisfy pridcate ok"
                    Success (first,remainingInput)
                else
                    printfn "satisfy pridcate KOOOO"
    ...
    // let rec everythingUntil parser input =
    //     // run parser with the input
    //     let firstResult = runOnInput parser input
    //     // test the result for Failure/Success
    //     match firstResult with
    //     | Failure _ ->
    //         let (subsequentValues,remainingInput) =
    //             everythingUntil parser inputAfterFirstParse
    //         let values = firstValue::subsequentValues
    //         (values,remainingInput)
    //         ([], input)
    //     | Success (firstValue,inputAfterFirstParse) ->
    ...
    // let until parser =
    //     let label = sprintf "until %s" (getLabel parser)
    //     let rec innerFn input =
    //         Success (everythingUntil parser input)
    //     {parseFn=innerFn; label=label}
